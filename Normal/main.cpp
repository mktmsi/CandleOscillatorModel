...
#define MAX_STEP (10000) //500000最大タイムステップ数700000
#define N (1)            //一辺のマス数,N*N＝セル数
//#define N0 (100)
void capture(int *);
//ディスプレイ
...
double eps = 0.001, au = 37.0, c = 5.0, sigma0 = 0.0, av = 0.1 * au, mu0 = 0.5;
...
int ParameterSetCount = 0, EndSetCount = 1;
...
void ParameterInitialization()
{
  ...
  { //シミュレーション開始時の処理
    ...
    count1 = ParameterSetCount % 8;
    count2 = ParameterSetCount / 8;
  ...
  //パラメータの更新
  //double u0Array[8] = {0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0};
  //double v0Array[8] = {0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0};
  //u0 = u0Array[count1];
  //v0 = v0Array[count2];
  u0 = 9.0 + (rand() * (11.0 - 9.0 + 1.0) / (1.0 + RAND_MAX));
  v0 = (5.0 + (rand() * (15.0 - 5.0 + 1.0) / (1.0 + RAND_MAX))) * 0.001;
  printf("Set=%d, coun1=%d, count2=%d, v0=%lf, u0=%lf\n", ParameterSetCount, count1, count2, v0, u0);
...
void FieldCycle()
{
  ...
  for (x = 0; x < N; x++)
  {
    ...
    if (N == 2)
    {
      u_new[x] += (FirstAndSecondTerm_u - ThirdTerm_u + ForthTerm_u) * dt;
